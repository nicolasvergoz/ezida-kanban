...
// newArchiveGetCmd builds `ezida archive get <id>`, reusing GetEnvelope
// but reading from the archive instead of the board.
func newArchiveGetCmd(jsonOut *bool) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "get <id>",
		Short: "Show the full detail of a single archived card",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runArchiveGet(cmd, board.ArchivePathFor(BoardPath), args[0], *jsonOut)
		},
	}
	return cmd
}

// runArchiveGet is the testable run body for `ezida archive get`. An
// archived card is not resolved against the live board's epic graph —
// editing means restoring first — so Epic/Children/Progress are always
// left empty; only the card's own fields plus ArchivedAt are reported.
func runArchiveGet(cmd *cobra.Command, archivePath, id string, asJSON bool) error {
	a, err := loadArchive(archivePath)
	if err != nil {
		return err
	}
	var found *board.ArchivedCard
	for i := range a.Cards {
		if a.Cards[i].ID == id {
			found = &a.Cards[i]
			break
	...
	if found == nil {
		return &CardNotArchivedError{ID: id}
	}
	...
	kvs := []output.KV{
		...
		{Key: "Archived", Value: found.ArchivedAt.UTC().Format("2006-01-02T15:04:05Z")},
