...
- [x] 1.1 In `internal/server/web/index.html`, replace the V3 modal body
...
- [x] 1.4 Keep the read-only metadata `<footer class="modal-readonly">`
...
- [x] 1.5 Keep the tag chips block but remove any V3 "Save tags" /
...
- [x] 2.1 In `internal/server/web/app.js`, replace the V3 modal state
...
- [x] 2.3 Implement `startEdit(name)`:
...
- [x] 2.4 Implement `commitField(name)`:
  - Read `drafts[name]`.
  - Call `await this.saveField(name, drafts[name])`.

- [x] 2.5 Implement `revertField(name)`:
...
- [x] 2.6 Implement `saveField(name, value)` per design MD4:
...
- [x] 2.7 Replace V3's `saveCard()` and any caller of it. Remove the
...
- [x] 3.1 Implement `onEscape()` on the modal overlay:
...
- [x] 3.2 Add `@keydown.escape.stop.prevent="revertField('<name>')"`
...
- [x] 3.4 Preserve the V4 external-change behavior: when the SSE
...
- [x] 4.3 Confirm tag PATCH rejection is surfaced via `errors.tags`
...
- [x] 5.1 In `internal/server/web/style.css`, add `.field-row` (column
...
- [x] 5.2 Add `.field--input`, `.field--textarea`, `.field--select`
...
- [x] 6.1 Run the existing V3 PATCH tests
...
- [x] 6.2 (Optional) Add a single structural test asserting the
...
- [x] 6.3 (Optional) Add a structural test asserting the rendered
...
- [x] 7.1 Click a card → modal opens with values rendered as plain
  text. No inputs visible until a field is clicked. Done when
  verified.

- [x] 7.2 Click the title → input swaps in, focused, with current
  value. Edit and blur → PATCH fires with `{"title":"..."}` only →
  rendered title updates from the server response. Done when
  verified.

- [x] 7.3 Clear the title → blur → editor stays open → inline error
...
- [x] 7.7 Esc with no field editing → modal closes, nothing changed
  on disk. Done when verified.

- [x] 7.8 Click overlay outside modal → modal closes, no prompt, no
  pending PATCH issued from this action. Done when verified.

- [x] 7.9 Open a card, edit title and blur (PATCH fires), then
  external CLI change to the same card → modal closes, board shows
  the merged state per last-write-wins. Done when verified.

## 8. Acceptance gate

- [x] 8.1 Run `go test ./... && go vet ./...`. Done when exit code is
