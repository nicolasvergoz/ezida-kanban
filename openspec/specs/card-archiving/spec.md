# Card Archiving Specification

## Purpose

Move finished or stale cards out of the live board into a sibling
`kanban.archive.toml` file rather than deleting them, so history survives
while `ezida list` and the board stay uncluttered. Covers the archive file
format, its relaxed validation, epic-cascading archive/unarchive, cross-file
write ordering, and the `ezida archive`/`ezida unarchive` CLI surface.
