...
The embedded HTML/JS viewer served by `ezida serve`. Renders the
kanban board from `/api/board`, lets the user mutate it via the
existing REST + SSE API, and surfaces the Redacto design system
(tokens, typography, theme toggle, filter, modal). Implementation
is React + Babel-standalone loaded from `/static/vendor/`.
## Requirements
